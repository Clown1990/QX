...
function insertNewsHtml(list, result){
	if(result == "SUCCESS"){
		var activityTableStr = "";
		for(var i = 0; i < list.length; i++){
			activityTableStr += "<li><div class='QXprovideContentListD QXprovideContentListDF'>" +
							"<a target='_blank' href='../QXactivityDetail.html?activityId='"+ list[i].activityId +"><h2>"+ list[i].title +"</h2></a>" +
							"<p class='PublishedTime'>"+ formatDate(list[i].time) +"</p>" +
							"<div class='QXprovideContentListC'>" +
							"<p>"+ getLengthProcessField(list[i].content) +"</p>" +
							"</div></div></li>";
		}
		$("#activityInfo").html(activityTableStr);
	}else
